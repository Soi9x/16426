...
public static class ItemEndpoints
{
    ...
    {
        // Trả về lỗi 2 giống như khi không tìm thấy hoặc chưa implement
    ...
    /// Implements logic EncodeLocations thực tế từ bản Go.
    /// </summary>
        ...
        ILogger<Program> logger)
    {
        ...
        // Lấy items của user hiện tại một lần
        var userItems = UserItems.GetOrAdd(userId, _ => new List<ItemData>());
        
        // Chuẩn bị danh sách locations duy nhất từ items của user (Logic EncodeLocations)
        // Trong Go: game.Items().EncodeLocations() trả về danh sách các location đã sở hữu
        var uniqueLocationIds = userItems
            .Select(i => i.LocationId)
            .Distinct()
            .Where(id => id != -1) // Loại bỏ location không hợp lệ
            .OrderBy(id => id)
            .ToList();

        // Mã hóa locations: [locationId, 0, 0, 0, 0, 0] (giả lập cấu trúc response của Go)
        // Lưu ý: Nếu Go trả về nhiều trường hơn cho location, cần bổ sung tại đây
        var encodedLocations = uniqueLocationIds.Select(locId => new object[]
        {
            locId,
            0, // Placeholder cho các trường khác của location nếu có
            0,
            0,
            0,
            0
        ...
        for (int j = 0; j < req.ProfileIds.Data.Count; j++)
        {
            ...
            // Trả về danh sách locations đã mã hóa cho profile của user
            // Các profile khác trả về mảng rỗng
            finalData[j] = (userId == profileId) ? encodedLocations : Array.Empty<object>();
        }
        ...
        ILogger<Program> logger)
    {
        ...
        if (req.ItemIds == null || req.Keys == null || req.Values == null || req.XpGains == null)
        {
            return Results.Ok(new object[] { 2, Array.Empty<object>(), Array.Empty<object>() });
        }
        ...
        ILogger<Program> logger)
    {
        ...
        {
             return Results.Ok(new object[] { 2 }); // Invalid request
        }
        ...
        foreach (var itemOrLocId in req.ItemOrLocIds)
        {
            ...
            if (!exists)
            {
                // Có thể là location ID, bỏ qua kiểm tra chặt chẽ ở đây
            }
        }
